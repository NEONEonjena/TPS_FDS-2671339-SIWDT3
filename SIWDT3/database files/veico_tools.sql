...
--
-- Table structure for table `outgoing_goods_one`
--

CREATE TABLE `outgoing_goods_one` (
  `id_outgoing_goods` char(16) NOT NULL,
  `id_user` int NOT NULL,
  `id_item` char(7) NOT NULL,
  `recipient_name` char(50) NOT NULL,
...
--
-- Triggers `outgoing_goods_one`
--
DELIMITER $$
CREATE TRIGGER `update_stock_out_one` BEFORE INSERT ON `outgoing_goods_one` FOR EACH ROW UPDATE `item` SET `item`.`stock` = `item`.`stock` - NEW.quantity_out WHERE `item`.`id_item` = NEW.id_item
$$
DELIMITER ;
...
(1, 'Administrador', 'admin', 'admin@email.com', '315-321-5743', 'admin', '$2y$10$7rLSvRVyTQORapkDOqmkhetjF6H9lJHngr4hJMSM2lHObJbW5EQh6', UNIX_TIMESTAMP(NOW()), 'user.png', 1);
...
--
-- Indexes for table `outgoing_goods_one`
--
ALTER TABLE `outgoing_goods_one`
  ADD PRIMARY KEY (`id_outgoing_goods`) USING BTREE,
  ADD KEY `id_user` (`id_user`) USING BTREE,
...
--
-- Constraints for table `outgoing_goods_one`
--
ALTER TABLE `outgoing_goods_one`
  ADD CONSTRAINT `outgoing_goods_one_fk1` FOREIGN KEY (`id_user`) REFERENCES `user` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `outgoing_goods_one_fk2` FOREIGN KEY (`id_item`) REFERENCES `item` (`id_item`) ON DELETE CASCADE ON UPDATE CASCADE;
